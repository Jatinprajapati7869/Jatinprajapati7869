<h1 align="center">Jatin Prajapati</h1>

<p align="center">
  <strong>AI/ML Engineer</strong>
</p>

<p align="center">
  <a href="mailto:jatinprajapati7869@gmail.com">
    <img src="https://img.shields.io/badge/Email-jatinprajapati7869%40gmail.com-D14836?style=flat-square&logo=gmail&logoColor=white" />
  </a>
  <a href="https://www.linkedin.com/in/jatin-prajapati-8840a0257">
    <img src="https://img.shields.io/badge/LinkedIn-Jatin%20Prajapati-0A66C2?style=flat-square&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://github.com/jatinprajapati7869">
    <img src="https://img.shields.io/badge/GitHub-jatinprajapati7869-181717?style=flat-square&logo=github&logoColor=white" />
  </a>
</p>

---

I build practical AI systems - from model training to API to user interface. My work focuses on computer vision, applied ML, and Japanese-language tools. I ship working prototypes with tests, CI, and documentation.

Currently studying at **IIT Madras** and interning at **Vicharshala, IIT Ropar**.

## Technical Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=python,ts,react,vite,fastapi,nextjs,tailwind,pytorch,docker,postgres,git,github,linux,rust" />
</p>

**ML / AI**: Python, PyTorch, YOLOv8, EfficientNet, Grad-CAM, MediaPipe

**Backend**: FastAPI, Next.js, Supabase, Prisma, SQLAlchemy

**Frontend**: React, TypeScript, Vite, Tailwind CSS, Framer Motion

**DevOps**: Docker, GitHub Actions CI, Husky, Commitlint

**Geospatial**: Google Earth Engine, OSMnx, Folium, Landsat 9

## Engineering Practices

Every pinned project follows professional standards:

- Passing builds and test suites
- CI pipelines (GitHub Actions)
- Typed codebases (TypeScript / Python type hints)
- Documented APIs and architecture
- MIT licensed with proper evaluation docs

## Current Focus

-  LLM applications with retrieval-augmented generation and evaluation
-  Computer vision systems for real-world deployment
-  Japanese language + engineering career in Japan
-  Building evaluation frameworks for ML model transparency

## Proof of Work

- **150+ LeetCode** problems solved - **AtCoder 979** rating
- Built 6+ production-quality AI/data projects from concept to deployment

## GitHub Snapshot

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=jatinprajapati7869&theme=github_dark" width="100%" />
</p>

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=jatinprajapati7869&theme=github_dark" width="49%" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=jatinprajapati7869&theme=github_dark" width="49%" />
</p>

---

<p align="center">
  <strong>Building useful AI systems with clear problem framing, tested code, and practical execution.</strong>
</p>
